---
name: opentodo-review-pr
description: Resolve Codex and independent subagent reviews and mark verified PRs ready. Use when opentodo-implement-issue reaches review or the user requests PR review and readiness.
---

Run directly with `$opentodo-review-pr <PR URL>` or as the review phase of `opentodo-implement-issue`. Either request includes the Codex review request, review fixes, commits/pushes, replies, thread resolution, and readiness unless the user narrows the scope. Merge only when explicitly requested.
...
1. Resolve the PR and review scope.
   ...
   - Read the PR, linked issue/spec, existing reviews, and repository instructions. For milestone work, fetch the canonical description with `gh api repos/OWNER/REPO/milestones/NUMBER` using the issue's milestone number. Resolve its working branch without disturbing unrelated changes. Stop if merged or closed. For an open ready PR, record its initial state, run `gh pr ready <number> --undo`, and verify draft state before proceeding. Restore readiness only after all gates below pass.
...
4. Mark the verified head ready.
   - Check the final body against the repository PR template under `.github/` when present. Otherwise describe the concrete problem, resulting behavior, and actual verification. Update it to match the delivered scope before readiness.
