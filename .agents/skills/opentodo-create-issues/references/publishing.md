...
2. Publish in dependency order.
   ...
   - Resolve the intended release milestone from the approved scope. Use `gh api` to read its canonical description and reuse the matching open milestone. When explicit issue publication is authorized and it does not exist, follow [milestone publication](../../opentodo-create-milestone/references/publishing.md) to create it with the agreed plan before publishing issues. Assign every in-scope issue to that milestone; do not reopen a closed milestone without authorization.
