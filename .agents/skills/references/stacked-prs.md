...
3. Propagate and verify changes.
   ...
   - Publish with `gh stack submit --auto --remote <name>`; new PRs are drafts. Apply the [repository commit/title convention](commit-conventions.md) to generated titles with `gh pr edit`. Use `gh stack sync --remote <name>` to reconcile remote changes; verify the resulting state even when the command exits successfully.
   - After prerequisite changes, rerun affected layers' checks. Return changed ready PRs to draft and resume their opentodo-review-pr checks before restoring readiness. Preserve review records so rebases cannot duplicate a Codex trigger within the run.
