...
Before doing any other Effect-related work, resolve the shared repo-explorer root and Effect source path:
...
Confirm that `EXPLORER_ROOT` is absolute and that `EFFECT_REPO` is a readable Git worktree. Treat the explorer checkout as read-only.
...
Only go directly to the shared Effect explorer checkout when:
...
3. Effect source code last. For gaps in the guides, complex type errors, unclear behavior, or implementation details, examine the shared Effect source at `$EFFECT_REPO/packages/effect/src/`.
...
- Prefer the most idiomatic Effect solution supported by the codebase and explorer source.
...
If no Effect patterns exist in the codebase, proceed using canonical patterns from the shared Effect source and examples. Do not block on missing codebase patterns.
...
- Resolve `$EFFECT_REPO/...` paths against the shared read-only explorer checkout.
...
These guides should be treated as the default implementation guidance. Do not skip them and jump straight to `$EFFECT_REPO` unless you need source-level confirmation or the guides do not answer the question.
...
When providing solutions, explain the Effect concepts being used and why they fit the specific use case. If you encounter patterns not covered in local references, prefer consistency with the codebase when possible and otherwise rely on the shared Effect explorer source.
