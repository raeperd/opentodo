---
name: opentodo-implement-issue
description: Implement a GitHub issue with behavior-focused TDD and small PRs, then compose opentodo-review-pr to deliver verified ready PRs.
disable-model-invocation: true
---

Run with `$opentodo-implement-issue <issue URL>`. This workflow includes implementation, verification, commits, pushes, draft PR creation, review through readiness, and a final PR handoff comment unless the user narrows the request. Merge only when explicitly requested.
...
1. Resolve scope and supporting skills.
   ...
   - Before designing or changing application logic, read [opentodo-effect-ts](../opentodo-effect-ts/SKILL.md) and the relevant guides. Identify which changes are effectful application logic and which are pure functions or framework UI code; apply any repository instructions in scope, including for new modules.
   - Follow the implementation and TDD steps below; [opentodo-review-pr](../opentodo-review-pr/SKILL.md) supplies the final independent review. All workflow procedures are repository-local.
   ...
   - If a concrete implementation uncertainty could benefit from a reference app or dependency example, run [opentodo-reference-research](../opentodo-reference-research/SKILL.md) before choosing the implementation approach or writing feature code. Carry source-backed findings into the plan and acceptance scenarios; skip this step when local code and accepted decisions suffice.
...
4. Verify and publish each layer as a draft.
   ...
   - Check for a repository PR template under `.github/` and follow it when present, including for bodies supplied through the CLI. Otherwise describe the concrete problem, resulting behavior, and actual verification. Keep the review fixed point in the run record for the review phase.
...
5. Run the opentodo-review-pr subworkflow.
   - Execute `opentodo-review-pr` for each completed draft without requiring another user prompt. Pass the PR URL, issue/spec, layer scope, and recorded review fixed point.
   ...
   - Completion: the layer is verified ready, or opentodo-review-pr leaves it in draft with a specific blocker.

6. Continue and report.
   - Repeat until all acceptance criteria are implemented. Rebase dependent layers after prerequisite changes, following `gh-stack`. Return affected ready PRs to draft and resume their opentodo-review-pr checks using the existing records before restoring readiness.
